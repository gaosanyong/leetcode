...
FROM (
    SELECT
        user_id,
        created_at - LAG(created_at, 1) OVER (PARTITION BY user_id ORDER BY created_at) AS diff
    FROM Users
) a
WHERE diff <= 7
...
SELECT
    flight_id,
    IFNULL(LEAST(total, capacity), 0) AS booked_cnt,
    IFNULL(GREATEST(0, total - capacity), 0) AS waitlist_cnt
FROM
    (SELECT flight_id, COUNT(*) AS total FROM Passengers GROUP BY 1) a
    RIGHT JOIN Flights USING (flight_id)
ORDER BY 1
...
| order_count | int  |
+-------------+------+
minute is the primary key for this table. Each row of this table contains the
minute and number of orders received during that specific minute. The total
number of rows will be a multiple of 6. Write a query to calculate total orders
within each interval. Each interval is defined as a combination of 6 minutes.
Minutes 1 to 6 fall within interval 1, while minutes 7 to 12 belong to interval
2, and so forth. Return the result table ordered by interval_no in ascending
...
+-------------+--------------+
Explanation:
- Interval number 1 comprises minutes from 1 to 6. The total orders in these six
  minutes are (0 + 2 + 4 + 6 + 1 + 4) = 17.
- Interval number 2 comprises minutes from 7 to 12. The total orders in these
  six minutes are (1 + 2 + 4 + 1 + 4 + 6) = 18.
...
| favorite_brand | varchar |
+----------------+---------+
seller_id is column of unique values for this table. This table contains seller
id, join date, and favorite brand of sellers.
...
| item_brand    | varchar |
+---------------+---------+
item_id is the column of unique values for this table. This table contains item
id and item brand.
...
| seller_id     | int     |
+---------------+---------+
order_id is the column of unique values for this table. item_id is a foreign key
to the Items table. seller_id is a foreign key to the Users table. This table
contains order id, order date, item id and seller id. Write a solution to find
the top seller who has sold the highest number of unique items with a different
brand than their favorite brand. If there are multiple sellers with the same
highest count, return all of them. Return the result table ordered by seller_id
...
- The user with seller_id 2 has sold three items, but only two of them are not
  marked as a favorite. We will include a unique count of 1 because both of
  these items are identical.
- The user with seller_id 3 has sold two items, but only one of them is not
  marked as a favorite. We will include just that non-favorite item in our
  count.
...
FROM
    (SELECT *, ROW_NUMBER() OVER () AS nu FROM Coordinates) a,
    (SELECT *, ROW_NUMBER() OVER () AS nu FROM Coordinates) b
WHERE a.X <= a.Y AND a.X = b.Y AND a.Y = b.X AND a.nu <> b.nu
ORDER BY 1, 2
...
WITH Cte AS (
    SELECT
        ...
        RANK() OVER (PARTITION BY city ORDER BY COUNT(*) DESC) rnk
    FROM Calls
    GROUP BY 1, 2
)
SELECT
...
WHERE number = 3 AND spend > prev_spend AND spend > prev_prev_spend
...
WITH Cte AS (
    ...
    SELECT a.user_id1, a.user_id2 AS common, b.user_id2
    FROM Cte a,
         Cte b
    WHERE a.user_id2 = b.user_id1 AND a.user_id1 <> b.user_id2
)
SELECT user_id1, user_id2 FROM Friends
WHERE (user_id1, user_id2) NOT IN (SELECT user_id1, user_id2 FROM Cte1)
ORDER BY 1, 2
...
SELECT
    transaction_date,
    IFNULL(SUM(IF(amount % 2 = 1, amount, 0)), 0) AS odd_sum,
    IFNULL(SUM(IF(amount % 2 = 0, amount, 0)), 0) AS even_sum
FROM transactions
GROUP BY 1
ORDER BY 1
...
| email           | varchar |
+-----------------+---------+
(user_id) is the unique key for this table. Each row contains a user's unique ID
and email address. Write a solution to find all the valid email addresses. A
...
* The part before the @ symbol contains only alphanumeric characters and
  underscores.
* The part after the @ symbol contains a domain name that starts with a letter
  and ends with .com.
...
| status_code | int     |
+-------------+---------+
log_id is the unique key for this table. Each row contains server access log
information including IP address and HTTP status code. Write a solution to find
invalid IP addresses. An IPv4 address is invalid if it meets any of these
conditions:
* Contains numbers greater than 255 in any octet
* Has leading zeros in any octet (like 01.02.03.04)
* Has less or more than 4 octets
Return the result table ordered by invalid_count, ip in descending order
respectively. The result format is in the following example.
...
FROM cte
WHERE
    CARDINALITY(octets) <> 4
    OR octets[1] ~ '^0' OR octets[1]::int > 255
    OR octets[2] ~ '^0' OR octets[2]::int > 255
    OR octets[3] ~ '^0' OR octets[3]::int > 255
    OR octets[4] ~ '^0' OR octets[4]::int > 255
GROUP BY 1
...
| description  | varchar    |
+--------------+------------+
(product_id) is the unique key for this table. Each row in the table represents
a product with its unique ID, name, and description. Write a solution to find
all products whose description contains a valid serial number pattern. A valid
serial number follows these rules:
* It starts with the letters SN (case-sensitive).
* Followed by exactly 4 digits.
* It must have a hyphen (-) followed by exactly 4 digits.
* The serial number must be within the description (it may not necessarily start
  at the beginning).
Return the result table ordered by product_id in ascending order. The result
format is in the following example.
...
* Product 3: Invalid serial number SN1234-56789 (contains 5 digits after the
  hyphen)
...
| species        | varchar |
+----------------+---------+
sample_id is the unique key for this table. Each row contains a DNA sequence
represented as a string of characters (A, T, G, C) and the species it was
collected from.
...
Return the result table ordered by sample_id in ascending order. The result
format is in the following example.
...
* For each pattern, 1 indicates the pattern is present and 0 indicates it is not
present*/
...
| department     | varchar |
+----------------+----------+
employee_id is the unique key for this table. Each row contains information
about an employee, including their ID, name, their manager's ID, salary, and
department. manager_id is null for the top-level manager (CEO). Write a solution
to analyze the organizational hierarchy and answer the following:
1. Hierarchy Levels: For each employee, determine their level in the
   organization (CEO is level 1, employees reporting directly to the CEO are
   level 2, and so on).
2. Team Size: For each employee who is a manager, count the total number of
   employees under them (direct and indirect reports).
3. Salary Budget: For each manager, calculate the total salary budget they
   control (sum of salaries of all employees under them, including indirect
   reports, plus their own salary).
Return the result table ordered by the result ordered by level in ascending
order, then by budget in descending order, and finally by employee_name in
ascending order. The result format is in the following example.
...
* Organization Structure:
    ...
    - Hank (ID: 8) reports to David, and Ivy (ID: 9) and Judy (ID: 10) report to
      Frank (level 4)
* Level Calculation:
...
* Within the same level, employees are ordered by budget in descending order
...
cte2 AS (
    SELECT  DISTINCT
        ...
        m.employee_name,
        m.level,
        COUNT(e.employee_id) OVER (PARTITION BY m.employee_id) AS team_size,
        COALESCE(SUM(e.salary) OVER (PARTITION BY m.employee_id), 0) + m.salary AS budget
    FROM cte m
    LEFT JOIN LATERAL (
        WITH RECURSIVE cte1  AS (
            ...
            SELECT
                ...
                ee.manager_id,
                ee.salary
            FROM cte1 mm, Employees ee
            WHERE ee.manager_id = mm.employee_id
        )
        SELECT * FROM cte1
    ) e ON 1 = 1
)
